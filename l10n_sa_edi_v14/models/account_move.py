...
class AccountMove(models.Model):
    ...
    def _compute_qr_code_str(self):
        ...
        for move in self:
            if move.country_code == 'SA' and move.move_type in ('out_invoice', 'out_refund') and move.l10n_sa_chain_index:
                edi_format = self.env.ref('l10n_sa_edi_v14.edi_sa_zatca')
                ...
                if move._l10n_sa_is_simplified():
                    ...
                    xml_content = self.env.ref('l10n_sa_edi_v14.edi_sa_zatca')._l10n_sa_generate_zatca_template(move)
    ...
    def _l10n_sa_get_qr_code(self, journal_id, unsigned_xml, x509_cert, signature, is_b2c=False):
        ...
        print("isueeeeeeee time and date=================",invoice_date,invoice_time)

        invoice_datetime = datetime.strptime(invoice_date + ' ' + invoice_time, '%Y-%m-%d %H:%M:%S')
        print("isueeeeeeee time and date=================222222222222222",invoice_datetime)
    ...
    def _l10n_sa_log_results(self, xml_content, response_data=None, error=False):
        ...
        """
        self.ensure_one()
        self.journal_id.sudo().l10n_sa_latest_submission_hash = self.env['account.edi.xml.ubl_21.zatca']._l10n_sa_generate_invoice_xml_hash(
            xml_content)

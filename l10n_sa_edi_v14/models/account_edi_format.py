...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_sa_get_digital_signature(self, company_id, invoice_hash):
        ...
        private_key = load_pem_private_key(company_id.sudo().l10n_sa_private_key, password=None,
    ...
    def _l10n_sa_calculate_signed_properties_hash(self, issuer_name, serial_number, signing_time, public_key):
        ...
        signed_properties = etree.fromstring(
            self.env.ref('l10n_sa_edi_v14.export_sa_zatca_ubl_signed_properties')._render({
        ...
        if clearance_data.get('json_errors'):
            ...
            for error in errors:
                ...
                for err in validation_results.get('errorMessages', []):
                    is_warning = False
                    error_msg += '\n - %s | %s' % (err['code'], err['message'])
            print("return data===========",error_msg,is_warning)
            return {
                'error': error_msg,
                'rejected': not is_warning,
                'blocking_level': 'warning' if is_warning else 'error'
            }
        print("clearance date=========",clearance_data, type(clearance_data))
    ...
    def _l10n_sa_export_zatca_invoice(self, invoice, xml_content=None):
        ...
        print("tes _l10n_sa_submit_einvoice ")
    ...
    def _l10n_sa_post_zatca_edi(self, invoice):  # no batch ensure that there is only one invoice
        ...
        if not invoice.l10n_sa_chain_index:
            ...
            # Retrieve the existing XML content from the invoice's attachments
            attachment = self.env['ir.attachment'].search([
                ('res_model', '=', 'account.move'),
                ('res_id', '=', invoice.id),
                ('name', 'like', 'Your XML Content Filename Pattern')  # Adjust this pattern to match your XML filenames
            ], limit=1)
            xml_content = attachment.datas.decode('utf-8') if attachment else None
        ...
        print("res datat----------",response_data,type(response_data))

        # Check for submission errors
        if type(response_data) != 'str' and response_data.get('error'):
    ...
    def _check_move_configuration(self, invoice):
        ...
        # if invoice.company_id.l10n_sa_api_mode == 'sandbox' and invoice.company_id.partner_id.commercial_partner_id.vat != '310175397400003':
        #     errors.append(
        #         _("- Please, make sure the VAT number of the company is equal to '310175397400003' when in Sandbox mode."))
    ...
    def _post_invoice_edi(self, invoices, test_mode=False):
        ...
        msg= self._l10n_sa_post_zatca_edi(invoice)
        print("msg===================",msg)
        return {invoice: msg}

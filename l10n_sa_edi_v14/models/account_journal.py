...
class AccountJournal(models.Model):
    ...
    l10n_sa_csr = fields.Binary(attachment=True, copy=False,
    ...
    def _l10n_sa_api_onboard_journal(self, otp):
        ...
        try:
            ...
            if not self.company_id.l10n_sa_private_key:
                ...
                # if self.company_id.l10n_sa_api_mode == 'sandbox' and self.company_id.partner_id.commercial_partner_id.vat != '310175397400003':
                #     raise UserError(_("- Please, make sure the VAT number of the company is equal to '310175397400003' when in Sandbox mode."))
    ...
    def _l10n_sa_get_production_CSID(self, OTP=None):
        ...
        renew = False
        zatca_format = self.env.ref('l10n_sa_edi_v14.edi_sa_zatca')
    ...
    def _l10n_sa_get_compliance_files(self):
        ...
        for file in file_names:
            fpath = get_module_resource('l10n_sa_edi_v14', 'tests/compliance', file)
    ...
    def _l10n_sa_run_compliance_checks(self):
        ...
        for fname, fval in compliance_files.items():
            invoice_hash_hex = self.env['account.edi.xml.ubl_21.zatca']._l10n_sa_generate_invoice_xml_hash(
                fval).decode()
            digital_signature = self.env.ref('l10n_sa_edi_v14.edi_sa_zatca')._l10n_sa_get_digital_signature(self.company_id, invoice_hash_hex).decode()
    ...
    def _l10n_sa_prepare_compliance_xml(self, xml_name, xml_raw, PCSID, signature):
        ...
        signed_xml = self.env.ref('l10n_sa_edi_v14.edi_sa_zatca')._l10n_sa_sign_xml(xml_content, PCSID, signature)
    ...
    def _l10n_sa_prepare_invoice_xml(self, xml_content):
        ...
        """
        ubl_extensions = etree.fromstring(self.env.ref('l10n_sa_edi_v14.export_sa_zatca_ubl_extensions')._render())
    ...
    def _l10n_sa_edi_get_next_chain_index(self):
        ...
        if not self.l10n_sa_chain_sequence_id:
            self.sudo().l10n_sa_chain_sequence_id = self.sudo().env['ir.sequence'].create({
                'name': f'ZATCA account move sequence for Journal {self.name} (id: {self.id})',
                'code': f'l10n_sa_edi_v14.account.move.{self.id}',
                'implementation': 'no_gap',
                'company_id': self.company_id.id,
            })
        return self.sudo().l10n_sa_chain_sequence_id.next_by_id()
    ...
    def _l10n_sa_api_get_pcsid(self):
        ...
        if not self.l10n_sa_production_csid_json:
            raise UserError(_("Please, make a request to obtain the Compliance CSID and Production CSID before sending "
                            "documents to ZATCA"))
        pcsid_validity = self.env.ref('l10n_sa_edi_v14.edi_sa_zatca')._l10n_sa_get_zatca_datetime(self.l10n_sa_production_csid_validity)
        time_now = self.env.ref('l10n_sa_edi_v14.edi_sa_zatca')._l10n_sa_get_zatca_datetime(datetime.now())

...
class DmsDirectory(models.Model):
    ...
    def open_files(self):
        _logger.info("===============open_files======")
        # if self.is_root_directory:
        #     action = self.env.ref('dms.action_dms_directories_directory').read()[0]
            
        #     action['name'] = str(self.name)
        #     action['domain'] = [('is_root_directory','=',False),('parent_id','=',self.id)]
        #     action['context'] = {'default_parent_id': self.id}
        # if self.is_root_directory == False:
        _logger.info("=====in idddddd==========open_files======")
        action = self.env.ref('dms.action_dms_file').read()[0]
        action['name'] = str(self.name)
        action['domain'] = [('directory_id','=',self.id)]
        action['context'] = {'default_directory_id': self.id}

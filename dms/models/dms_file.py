...
class File(models.Model):
    ...
    arch_no = fields.Char('Arch No')
    website = fields.Char('Website')
    site_details = fields.Text('Site Details')
    instrument_no = fields.Char('instrument No')
    instrument_owner = fields.Char('instrument owner')
    owner_per = fields.Char('Owner Percentage')
    book_value = fields.Char('Book Value')
    market_value = fields.Char('Market Value')
    notes = fields.Text('Notes')
    instrument_date = fields.Date('Instrument Date')
    instrument_space = fields.Float('Instrument Space')
    no_planned = fields.Char('No. planned')
    name_of_r = fields.Char('The name of the registered owner of the deed')
    attach_ids = fields.One2many('dms.file.doc','file_id')
    ...
    @api.onchange('instrument_date')
    def onchange_date(self):
        _logger.info("================onchange_date======")
        if self.instrument_date:
            date = self.instrument_date
            xx = Gregorian(date.year,date.month,date.day).to_hijri()
            self.hijri_date = str(xx)
            _logger.info("================onchange_date======"+str(xx))

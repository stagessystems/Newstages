    ...
    def check_budget(self):
        for line in self.product_line_ids:
            ...
            if account:
                ...
                budget_lines = self.env['crossovered.budget.lines'].search([('date_from','<=',self.date_confirmed.date()),('date_to','>=',self.date_confirmed.date())])
                _logger.info("----------------"+str(budget_lines))
                for budget in budget_lines:
                    for acc in budget.general_budget_id.account_ids:
                        if account.id == acc.id:
                            _logger.info("============Ohhh"+str(acc)+"=="+str(budget.percentage))
                            if budget.percentage < -1:
                                raise UserError(_("You out of budget."))
